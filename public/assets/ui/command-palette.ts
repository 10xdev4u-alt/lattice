...
export function mountCommandPalette(): void {
  ...
  const commands: Command[] = [
    ...
    { id: 'share', label: 'Share session URL', hint: 'copy to clipboard', keywords: 'share url copy session', run: () => void (async () => { const { buildShareUrlAsync } = await import('../share'); const { notice } = await import('./overlays'); const u = await buildShareUrlAsync(); void navigator.clipboard?.writeText(u).catch(() => undefined); await notice('Share URL copied', u); })() },
