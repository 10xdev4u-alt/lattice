...
export function mountCommandPalette(): void {
  ...
  const commands: Command[] = [
    ...
    { id: 'share', label: 'Share session URL', hint: 'copy to clipboard', keywords: 'share url copy session', run: () => void import('../share').then((m) => { const u = m.buildShareUrl(); void navigator.clipboard?.writeText(u).catch(() => { /* the prompt below shows the URL either way */ }); window.prompt('Share URL (copied):', u); }) },
