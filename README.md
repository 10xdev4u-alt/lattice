...
| **LLM** | OpenAI-compatible endpoint at `https://api.kilo.ai/api/gateway/v1`, live free-model pool (head: `liquid/lfm-2.5-2.6b:free`), key `latticex` (swap via env var); browser calls route through `/api/llm`; offline fallback via in-browser WebLLM (Phi-3-mini) |
